...
function OrderSetupModule() {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      const companyRes = await fetch(`${BACKEND_URL}/api/companies/single`);
      if (companyRes.ok) {
  ...
  // 🖨️ FRONTEND BROWSER PRINT DIALOG LOGIC
  const triggerBrowserPrint = (orderData, tokenNo) => {
    const selectedWaiterObj = waiters.find(w => Number(w.id) === Number(orderData.waiter_id));
    const selectedTableObj = tables.find(t => Number(t.id) === Number(orderData.table_id));
    
    const companyName = companyInfo?.company_name || 'MY RESTAURANT';
    const address = `${companyInfo?.address1 || ''} ${companyInfo?.address2 || ''}`;
    const gstNo = companyInfo?.gst_no ? `GSTIN: ${companyInfo.gst_no}` : '';
    ...
    let printHTML = `
      ...
      <head>
        ...
        <style>
          ...
          body { width: 80mm; margin: 0; padding: 5px; font-family: 'Hind Madurai', sans-serif; font-size: 12px; color: #000; }
          ...
          .total-section { font-size: 13px; margin-top: 5px; }
        </style>
      </head>
      <body>
        <div class="center">
        ...
        <table class="info-table">
          <tr>
            <td><b>Order:</b> ${orderData.order_pfx}${orderData.order_no}</td>
            <td class="right"><b>Table:</b> ${selectedTableObj ? selectedTableObj.table_no : 'N/A'}</td>
          </tr>
          <tr>
            <td><b>Date:</b> ${orderData.order_date}</td>
            <td class="right"><b>Time:</b> ${orderData.order_time}</td>
          </tr>
          <tr>
            <td><b>Waiter:</b> ${selectedWaiterObj ? selectedWaiterObj.waiter_name : 'N/A'}</td>
            <td class="right"><b style="border: 1px solid #000; padding: 1px 3px;">TOKEN: ${tokenNo}</b></td>
          </tr>
        </table>
        <div class="line"></div>
    ...
    printHTML += `
        ...
        <table class="info-table total-section">
          <tr class="bold">
            ...
            <td class="right">Gross: ₹${Number(orderData.gross_value).toFixed(2)}</td>
          </tr>
          ${Number(orderData.gst_value) > 0 ? `
          <tr>
            <td></td>
            <td class="right">GST (${orderData.gst_percent}%): ₹${Number(orderData.gst_value).toFixed(2)}</td>
          </tr>` : ''}
          <tr class="bold" style="font-size: 14px;">
            <td></td>
            <td class="right">NET: ₹${Number(orderData.net_value).toFixed(2)}</td>
          </tr>
        </table>
        <div class="line"></div>
    ...
    // Create Hidden Iframe for Standard System Printer Dialog
  ...
  const saveOrder = async (isPrint = true) => {
    ...
    try {
      ...
      if (res.ok) {
        if (isPrint) {
          triggerBrowserPrint(orderData, resData.token_no || '001');
        } else {
